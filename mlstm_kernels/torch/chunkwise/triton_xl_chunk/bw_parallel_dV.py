...
from ....triton.chunkwise.xl_chunk import mlstm_chunkwise__parallel_bw_dV_kernel, mlstm_chunkwise__recurrent_bw_dV_kernel
...
def mlstm_chunkwise__parallel_bw_dV(
...
]:  # matDeltaV (B, NH, S, DHHV), vecAux (B, NH, S, 1)
    ...
    return matDeltaV, vecAux


def mlstm_chunkwise__recurrent_bw_dV(
    ## Forward arguments
    matQ: torch.Tensor,  # (B, NH, S, DHQK)
    matK: torch.Tensor,  # (B, NH, S, DHQK)
    matV: torch.Tensor,  # (B, NH, S, DHHV)
    ...
    mlstm_chunkwise__recurrent_bw_dV_kernel[grid](
        matK=matK,
        vecA=vecA,
        ...
        str_matHV_S=matV.stride(2),
        str_matHV_DHHV=matV.stride(3),
        str_vecABI_B_NH=vecA.stride(1),
        str_vecABI_NC=vecA.stride(2),
