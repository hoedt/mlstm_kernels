...
from ....triton.chunkwise.limit_chunk import mlstm_chunkwise__parallel_bw_dQKV_kernel, mlstm_chunkwise__recurrent_bw_dQKV_kernel
...
def mlstm_chunkwise__parallel_bw_dQKV(
...
]:  # matDeltaQ (B,NH,S,DHQK), matDeltaK (B,NH,S,DHQK), matDeltaV (B,NH,S,DHHV), vecAux (B, NH, S, 1)
    ...
    siz_b_DHQK = get_head_dim_block_size(
        head_dim=DHQK, min_block_size=32 if _dtype == torch.float32 else 64
    )
    siz_b_DHHV = get_head_dim_block_size(
        head_dim=DHHV, min_block_size=32 if _dtype == torch.float32 else 64
    )
    ...
    vecAux = torch.zeros((B, NH, S, 1), dtype=_dtype, device=_device)
    ...
    return matDeltaQ, matDeltaK, matDeltaV, vecAux


def mlstm_chunkwise__recurrent_bw_dQKV(
    matK: torch.Tensor,  # (B, NH, S, DHQK)
    matV: torch.Tensor,  # (B, NH, S, DHHV)
    vecB: torch.Tensor,  # (B, NH, NC, L)
    vecI: torch.Tensor,  # (B, NH, NC, L)
    ...
    grid = (num_b_DHQK, NC, B * NH)
    # print(f"grid: {grid}")
    mlstm_chunkwise__recurrent_bw_dQKV_kernel[grid](
        matK=matK,
        matV=matV,
        vecB=vecB,
        vecI=vecI,
        ...
        matDeltaV=matDeltaV,
        qk_scale=qk_scale,
        str_matQK_B_NH=matK.stride(1),
        str_matQK_S=matK.stride(2),
        str_matQK_DHQK=matK.stride(3),
        str_matDV_num_b_DHQK=matDeltaV.stride(0),
        str_matHV_B_NH=matV.stride(1),
        str_matHV_S=matV.stride(2),
        str_matHV_DHHV=matV.stride(3),
        ...
        str_scaM_inter_B_NH=scaM_inter.stride(1),
        str_scaM_inter_NC=scaM_inter.stride(2),
        str_vecN_states_B_NH=vecN_states.stride(1),
        str_vecN_states_NCDHQK=vecN_states.stride(2),
        str_matC_states_B_NH=matC_states.stride(1),
        str_matC_states_NCDHQK=matC_states.stride(2),
        str_matC_states_DHHV=matC_states.stride(3),
        str_vecL_out_B_NH=vecL_out.stride(1),
