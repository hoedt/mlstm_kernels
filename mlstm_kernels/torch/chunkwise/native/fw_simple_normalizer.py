...
Forward pass of the mLSTM chunkwise formulation with simplified normalizer.
...
def mlstm_chunkwise__parallel_fw_H(
    ...
    matH_k_chunk = matNumerator_common / (vecDenom_l_common + eps)

    matH_out = rearrange(matH_k_chunk, "b nh nc l dh -> b nh (nc l) dh")

    # return the denominator and max state for compatibility
    vecL_out = rearrange(vecDenom_l_common, "b nh nc l 1 -> b nh (nc l)")  # (B, NH, S)
